...
| `validation/collections.ts` | C1–C13 collection validators + `sanitizePostBody()` for XSS |
| `post-blocks.ts` | Closed list of highlighted-block roles (callout / chat examples) — shared by sanitizer, editor and CSS |
| `renderer/post-toc.ts` | `injectHeadingIds()` + `renderPostToc()` — anchors in H2 and the article table of contents |
...
### Collection validators (C1–C13) + sanitizer
...
### Highlighted blocks in post bodies (`post-blocks.ts`)

`src/lib/cms/post-blocks.ts` is the **single source of truth** for the closed list of block roles a client can apply inside an article — currently `f-callout` ("W praktyce"), `f-chat--risk`, `f-chat--safe`. It has no imports on purpose: `RichTextEditor.tsx` is a client component and `collections.ts` pulls in `sanitize-html` (a node lib), so the shared constant must live outside both.

Three places must agree, and all three read from it:
1. `allowedClasses` in `sanitizePostBody` — the only classes that survive a save
2. the toolbar buttons + paste normalisation in `RichTextEditor.tsx`
3. the `.f-callout` / `.f-chat--*` rules in `forma-components.css`

The class carries a **role**, never a look: the visible labels ("Ryzykowny komunikat", "Bezpieczniejsze brzmienie") are generated by CSS `::before`, so a client cannot mistype them or invent a colour. `class` is allowlisted on `blockquote` only — it is stripped from every other tag.

### Article table of contents and sources

`renderer/post-toc.ts` — `injectHeadingIds()` rewrites every `<h2>` in `post.body` with an anchor id and returns the heading list; `renderPostToc()` builds the `<details open>` navigation (suppressed below two headings). Ids are computed **at render time and never stored**, so renaming a section moves the link and the label together. The regex rewrite is safe because `sanitizePostBody` allows no attributes on `h2`, so stored headings are always bare.

`PostItem.sources?: string[]` renders as a separate panel at the end of the article. Unlike the rest of the renderer, source text is **escaped** (`escapeHtml` in `renderer/utils.ts`) and bare `http(s)` URLs are turned into links by splitting the raw string before escaping — do not switch this to raw interpolation.
