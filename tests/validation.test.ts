...
describe('V16 — pola kontaktowe', () => {
  it('fixture bez zmian → 0 naruszeń V16', () => {
    const model = loadFixture()
    expect(validateHard(model).filter(v => v.rule === 'V16')).toHaveLength(0)
  })

  it('pusty contactPhone → błąd na meta.contactPhone', () => {
    const m = clone(loadFixture())
    m.meta.contactPhone = ''
    const errs = validateHard(m).filter(v => v.rule === 'V16')
    expect(errs.length).toBeGreaterThan(0)
    expect(errs.some(e => e.field === 'meta.contactPhone')).toBe(true)
  })

  it('za krótki contactPhone (3 cyfry) → błąd', () => {
    const m = clone(loadFixture())
    m.meta.contactPhone = '123'
    const errs = validateHard(m).filter(v => v.rule === 'V16')
    expect(errs.some(e => e.field === 'meta.contactPhone')).toBe(true)
  })

  it('poprawny numer +48668902855 → brak błędu V16 dla telefonu', () => {
    const m = clone(loadFixture())
    m.meta.contactPhone = '+48668902855'
    const errs = validateHard(m).filter(v => v.rule === 'V16' && v.field === 'meta.contactPhone')
    expect(errs).toHaveLength(0)
  })

  it('pusty contactPhoneDisplay → błąd na meta.contactPhoneDisplay', () => {
    const m = clone(loadFixture())
    m.meta.contactPhoneDisplay = ''
    const errs = validateHard(m).filter(v => v.rule === 'V16')
    expect(errs.some(e => e.field === 'meta.contactPhoneDisplay')).toBe(true)
  })

  it('nieprawidłowy email → błąd na meta.contactEmail', () => {
    const m = clone(loadFixture())
    m.meta.contactEmail = 'niemail'
    const errs = validateHard(m).filter(v => v.rule === 'V16')
    expect(errs.some(e => e.field === 'meta.contactEmail')).toBe(true)
  })

  it('poprawny email → brak błędu V16 dla emaila', () => {
    const m = clone(loadFixture())
    m.meta.contactEmail = 'kontakt@forma.pl'
    const errs = validateHard(m).filter(v => v.rule === 'V16' && v.field === 'meta.contactEmail')
    expect(errs).toHaveLength(0)
  })
})
