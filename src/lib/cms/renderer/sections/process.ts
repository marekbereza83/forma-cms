...
export function renderProcess(section: Section, linkMode: 'static' | 'preview' = 'static', lang: Lang = 'pl'): string {
  const s = t(lang)
...
<section id="process" class="section bg-surface reveal" aria-labelledby="process-heading">
  <div class="container">
    <div class="section-header">
      <span class="section-label">${s.process.label}</span>
      <h2 id="process-heading" class="f-headline">${headline}</h2>
    </div>

    <ol class="process-list-wrapper stagger-reveal" aria-label="${s.process.stepsAria}">
${stepsHtml}
    </ol>

    <a href="${procesHref}" class="section-cta-link mt-10"
      aria-label="${s.process.fullProcessAria}">
      ${s.process.fullProcess}
      ${ARROW_ICON}
    </a>
  </div>
</section>`
