...
interface RenderContext {
  ...
  // true dla stron z utility-footer (kontakt, legal): bez <nav> wrappera, aria-current="page"
  showCurrentInFooter: boolean
}

const SECTION_REGISTRY: Record<string, (s: Section, ctx: RenderContext) => string> = {
  // ── index ────────────────────────────────────────────────────────────────────
  'nav':               (s, ctx) => renderNav(s, ctx.currentPage, ctx.linkMode, ctx.navPages, ctx.showCurrentInFooter),
  ...
  'footer':            (s, ctx) => renderFooter(s, ctx.currentPage, ctx.linkMode, ctx.showCurrentInFooter),
  // ── portfolio ────────────────────────────────────────────────────────────────
...
export function renderPage(model: SiteModel, slug: string, basePath = '', linkMode: 'static' | 'preview' = 'static'): string {
  ...
  const ctx: RenderContext = { basePath, pricingStandardAmount, currentPage: slug, linkMode, navPages, indexPricing, showCurrentInFooter }
