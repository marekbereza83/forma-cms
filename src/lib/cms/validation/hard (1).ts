...
export function validateHard(model: SiteModel): Violation[] {
  ...
  // ── V15: portfolio-grid card links must use http:// or https:// ──────────────
  // Blocks: javascript:, data:, //, and any other non-http scheme (XSS vector).
  // Empty string = no link → skip validation.
  if (portfolioGridSection) {
    const cardsField = portfolioGridSection.fields['cards']
    if (Array.isArray(cardsField?.value)) {
      const cards = cardsField.value as PortfolioCard[]
      cards.forEach((card, i) => {
        if (card.link && !SAFE_URL_RE.test(card.link)) {
          errors.push({
            rule: 'V15',
            field: `portfolio-grid.cards[${i}].link`,
            message: `Link karty "${card.title || String(i + 1)}" musi zaczynac sie od https:// lub http:// (niedozwolony schemat: ${card.link.split(':')[0]})`,
          })
        }
      })
    }
